<script>
	...
	import { configuredButtonKey, splitRowMenuButtons } from '../../../utils/buttonConditions.ts';
	...
	let rowMenuSplit = $derived(splitRowMenuButtons(rowMenuItems, menuItemsVisible));
	...
	>
		{#each visibleRows as visibleRow}
			...
			{@const rowButtons = tableAPI.resolveRowButtons(inlineButtons, row, {
				forceDisabled: inInsert || rowState.editing == visibleRow || meta?.disabled
			})}
			...
			>
				<div class="row-buttons" style:gap={inlineButtons.length > 1 ? '0.25rem' : '0rem'}>
					{#if rowMenu && rowButtons?.length}
						{#each rowButtons as button, index (configuredButtonKey(button, index))}
							<SuperButton {...button} onClick={() => button.onClick?.()} />
						{/each}
					{/if}
...
{#if openMenu}
	...
	{@const overflowButtons = tableAPI.resolveRowButtons(menuItems, menuRow, {
		forceDisabled: inInsert || rowState.editing == rowState.menuRow || menuMeta?.disabled
	})}
	<SuperPopover
		open
		anchor={menuAnchor}
