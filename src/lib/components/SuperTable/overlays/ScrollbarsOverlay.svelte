	...
	on:mousemove={dragging || horizontalDragging
		? (e) => {
				if (dragging) {
					e.preventDefault();
					e.stopPropagation();
					mouseoffset = (e.clientY - startPos) * (scrollHeight / clientHeight) + startScrollPos;
					if (mouseoffset > 0 && mouseoffset <= verticalRange) scrollPos = mouseoffset;
				...
				if (horizontalDragging) {
					...
					e.stopPropagation();
					mouseoffset =
						(e.clientX - horizontalStartPos) * (anchor?.scrollWidth / anchor?.clientWidth) +
						startScrollPos;
					if (mouseoffset > 0 && mouseoffset <= horizontalRange) horizontalScrollPos = mouseoffset;
				}
