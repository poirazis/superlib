<script>
	...
	let { cellOptions, value, fieldSchema, multi = true, autofocus = false } = $props();
	...
	let fullSelection = $derived(
		source.filteredOptions.length == localValue.length && source.filteredOptions.length > 0
	);
	let radios = $derived(controlType == 'radio');
	let isButtons = $derived(controlType == 'buttons');
	let allSelected = $derived(source.filteredOptions.length == localValue.length);
				...
				{:else if controlType == 'switch'}
					...
					>
						{#if cellOptions.toggleAll}
							<div class="switch toggle-all" on:mouseenter={() => (focusedOptionIdx = -1)}>
...
>
	<CellPickerOptionsList bind:optionsList={picker} onMouseLeave={() => (focusedOptionIdx = -1)}>
...
<style>
	...
	.switch > .spectrum-Switch {
		...
		--spectrum-switch-m-handle-border-color: var(--spectrum-global-color-gray-500) !important;
	}
