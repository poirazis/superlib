<script lang="ts">
	...
	const getInitialValues = (
		...
		for (let id of path.toReversed().slice(1)) {
			if (dataSource.type === 'viewV2' && context[id]?._viewId === dataSource.id) {
				return context[id];
			}
	...
	const generateSchemaKey = (schemaValue: TableSchema | undefined) => {
		...
		fields.sort();
		return fields.map((field) => `${field}:${schemaValue[field].type}`).join('-');
	};
	...
	let initialValues = $derived(
		getInitialValues(actionType, dataSource, $component.path, $context, row)
	);
	let resetKey = $derived(hashString(schemaKey + JSON.stringify(initialValues) + actionType));
</script>
