<script>
  ...
  let effectiveTrackFill = $derived(
    trackFillColor ? trackFillColor : "var(--primaryColor)",
  );
  let _textBaseline = $derived(
    arcSize == 180 || arcSize == 120 ? "bottom" : "middle",
  );
  let arc = $derived(
    d3arc()
  ...
  >
    ...
    <defs>
      <linearGradient id={_id} x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <stop
          offset="100%"
          stop-color={trackGradientColor || effectiveTrackFill}
        />
      </linearGradient>
    </defs>
  </svg>
